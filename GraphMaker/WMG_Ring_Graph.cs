...
public class WMG_Ring_Graph : WMG_Graph_Manager {
	...
	void ToggleComputeShaderTexture(GameObject obj, Sprite sprite) {
		if (useComputeShader) {
			...
			if (cs == null) {
				...
				tex.flip = UIBasicSprite.Flip.Both;
				tex.fillAmount = 1;
			...
			if (cs != null) {
				DestroyImmediate(cs);
				UITexture tex = obj.GetComponent<UITexture>();
				tex.flip = UIBasicSprite.Flip.Nothing;
	...
	void updateLabelLineBasedOnOverlap(WMG_Ring ring, int numOverlapping) {
		changeSpriteWidth (ring.textLine, Mathf.RoundToInt(4 + getSpriteHeight(ring.labelBackground) * numOverlapping));
	...
	void updateLabelRotationAndPosition(WMG_Ring ring, float degOffset, int numOverlapping, bool onComplete) {
		...
		if (labelOnRightSide) {
			...
			UISprite labelBgSprite = ring.labelBackground.GetComponent<UISprite> ();
			labelBgSprite.leftAnchor.Set (0, -labelBgSprite.width);
			labelBgSprite.rightAnchor.Set (0, 0);
			...
			UISprite labelBgSprite = ring.labelBackground.GetComponent<UISprite> ();
			labelBgSprite.leftAnchor.Set (0, 4);
			labelBgSprite.rightAnchor.Set (0, 4 + labelBgSprite.width);
