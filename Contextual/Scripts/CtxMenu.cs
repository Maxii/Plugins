...
public class CtxMenu : MonoBehaviour {
    ...
    public void Show(Vector3 screenPos, Item[] itemsArray) {
        //Debug.Log("CtxMenu.Show() "+name+" "+screenPos+" "+itemsArray.Length);
                ...
                if (item.isDisabled) {
                    ...
                    if (style == Style.Pie) {
                        if (itemData[i].background != null) {
                            ...
                            if (itemData[i].background.GetComponent<Collider>() != null)
                                itemData[i].background.GetComponent<Collider>().enabled = false;
                        }
                    }
                    else {
                        if (itemData[i].highlight != null && itemData[i].highlight.GetComponent<Collider>() != null)
                            itemData[i].highlight.GetComponent<Collider>().enabled = false;
                    }
                    ...
                    if (style == Style.Pie) {
                        if (itemData[i].background != null) {
                            ...
                            if (itemData[i].background.GetComponent<Collider>() != null)
                                itemData[i].background.GetComponent<Collider>().enabled = true;
                            else
                        ...
                        if (itemData[i].highlight != null) {
                            if (itemData[i].highlight.GetComponent<Collider>() != null)
                                itemData[i].highlight.GetComponent<Collider>().enabled = true;
                            else
            ...
            if (UICamera.selectedObject != gameObject && index == -1 && currentSubmenu == null) {
                ...
                if (parentMenu != null && parentMenu.IsSubmenuItemSelected(this)) {
                    ...
                    if (pendingHide) {
                        ...
                        //Debug.Log("CtxMenu.LateUpdate() - "+name+" wants to hide [sel = "+UICamera.selectedObject+"] [index = "+index+"]");
                        pendingHide = true;
                    }
                }
            }
    ...
    void OnLocalize() {
        if (isLocalized && language != Localization.language && itemData != null && items != null) {
    ...
    void ShowSubmenu(int itemIndex) {
        //Debug.Log("CtxMenu.ShowSubmenu() "+itemIndex+" "+currentSubmenu);
...
#else
            currentSubmenu.Show(submenuPos, submenu.items);
#endif
        }
    }
...
#else
 {
    ...
    void SetHighlight(int newIndex) {
        if (items == null || items.Length == 0 || index == newIndex)
            return;
        ...
        //Debug.Log("CtxMenu.SetHighlight() "+name+" "+index+" "+openSubmenu);
