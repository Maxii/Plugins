...
public class GFPolarGrid : GFLayeredGrid {
	...
	/// are. The value cannot go below `Mathf.Epsilon`.
	public float radius { get{ return _radius;}
		set {
			SetMember<float>(value, ref _radius, restrictor: Mathf.Max, limit: Mathf.Epsilon);
		}
	}
	...
	/// angle within a sector in radians and it is a shorthand writing for
	...
	/// <returns>Rotation quaternion which rotates around the origin.</returns>
	/// <param name="sector">Sector coordinate inside the grid.</param>
	/// 
	/// This is basically the same as <see cref="Angle2Rotation"/>, except with
	...
	/// @internal <summary>
	///   Interprets a float as degree; loops if value exceeds 360 and runs in
	///   reverse for negative values.
	/// </summary>
	...
	/// <param name="worldPoint">Point in world space.</param>
	/// 
	/// Returns the position of the nearest vertex in polar coordinates from a
	/// given point in world space.
	...
	public Vector3 NearestFaceP(Vector3 worldPoint) {
		...
		// virtually shift the point half an angle and half a radius down, this
		// will simulate the shifted coordinates
	...
	public Vector3 NearestBoxP(Vector3 worldPoint) {
		Vector3 polar = WorldToPolar(worldPoint);
		// virtually shift the point half an angle, half a radius and half the
		// depth down, this will simulate the shifted coordinates
	...
	#region overload
	/// @overload
	/// Use this overload when you want to apply the alignment on all axis,
	/// then you don't have to specify them.
